...
export async function GET(req: NextRequest) {
  ...
  const settingsUrl = `${origin}/settings?wearable=whoop`
  ...
  try {
    ...
    if (!parsed || parsed.provider !== 'whoop') {
      return NextResponse.redirect(`${settingsUrl}&error=invalid_state`)
    }
    ...
    const tokens = await exchangeWhoopAuthCode(code)
    await saveTokens(session.user.id, 'whoop', tokens)

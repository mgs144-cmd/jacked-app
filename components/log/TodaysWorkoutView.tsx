...
export function TodaysWorkoutView({
  ...
  const [templateJson, setTemplateJson] = useState<WorkoutDayTemplateJson>(createEmptyTemplateDay().template_json)
  ...
  const startActionCopy = useMemo(() => {
    if (!mounted) {
      return { title: 'Start workout', description: "Today's full session" }
    }
    const date = selectedCalDate ?? todayISO()
    if (date === todayISO()) {
      return { title: 'Start workout', description: "Today's full session" }
    }
    return {
      title: 'Log workout',
      description: formatWorkoutDateAbsolute(date),
    }
  }, [mounted, selectedCalDate])

  const calendarHint = useMemo(() => {
    if (!mounted) return 'View past sessions'
    const count = workoutDays.length
    if (count === 0) return 'View past sessions'
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <div suppressHydrationWarning>
        <LogHubActions
          startTitle={startActionCopy.title}
          startDescription={startActionCopy.description}
          onStartWorkout={() => onStartWorkout(selectedCalDate ?? todayISO())}
            ...
            {programs.map((row) => {
              ...
              return (
                <div key={row.id} className="rounded-xl border border-white/[0.08] bg-black/20 p-3">
                  ...
                  <p className="log-screen-meta text-xs">{json.days.length} day{json.days.length !== 1 ? 's' : ''}</p>
                </div>
              )
            })}
          </div>
